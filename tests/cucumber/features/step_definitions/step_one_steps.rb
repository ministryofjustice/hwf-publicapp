...
When(/^I select I don’t know the form name or number$/) do
  form_name = step_one_page.content.new_form_name
  expect(form_name.form_group[1].block_label[0].text)
    .to eq 'I don’t know the form name or number'
...
When(/^I select I need help with an employment tribunal fee$/) do
  form_name = step_one_page.content.new_form_name
  expect(form_name.form_group[2].block_label[0].text)
    .to eq 'I need help with an employment tribunal fee'
  expect(form_name.form_group[2].form_name_et['type']).to eq 'checkbox'
  form_name.form_group[2].form_name_et.click
  form_name.form_group[4].continue_button.click
end
...
Then(/^I should see the step one header$/) do
  expect(step_one_page.content.page_header.text)
    .to eq 'What court or tribunal fee do you need help with?'
end

Then(/^I should see step one hint$/) do
  expect(step_one_page.content.hint.text)
    .to have_content 'Enter the court or tribunal form number'
end
...
Then(/^I see error message telling me I need to fix the errors$/) do
  ...
  expect(error_summary.error_link.text)
    .to have_content 'Enter the form name or number'
  expect(step_one_page.content.new_form_name.form_group[0].error_message.text)
    .to have_content 'Enter the form name or number'
end
...
Then(/^I should not see the cancel application options$/) do
  expect(step_one_page.restart_confirm.button[0])
    .to_not be_visible
  expect(step_one_page.restart_confirm.button[1])
    .to_not be_visible
end
