...
Then(/^I should see the step two header$/) do
  expect(step_one_page.content.page_header.text)
    .to eq 'Have you already paid the fee?'
end

Then(/^I should see step two hint$/) do
  expect(step_two_page.content.text.p.text)
    .to have_content 'apply for a refund for a fee paid in the last 3 months'
end
...
Then(/^I should see date exceeding 3 months error message$/) do
  ...
  expect(error_summary_group.error_summary_heading.text)
    .to eq 'You need to fix the errors on this page before continuing.'
  expect(error_summary_group.link.text)
    .to eq 'The application must have been made in the last 3 months'
...
And(/^the date I enter is within the last three months$/) do
  ...
  expect(fee_date_paid_group.hint.text)
    .to eq 'Use this format D D / M M / Y Y Y Y'
